...
async def simulate_batch():
 ...
 @app.post("/api/defend")
 async def defend_system():
     """Initiates simulated defense protocols across all layers."""
     # Simulate backend hardening
     time.sleep(1) # Processing delay
     return {
         "success": True, 
         "message": "Network perimeter secured. Threat status: CONTAINED.",
         "protocol": "Nexus-Llama-Shield",
         "actions": [
             "Rogue IPs blacklisted",
             "Ingestion pipeline hardened",
             "Memory scrubbing completed"
         ]
     }

# In-memory vulnerabilities
vulnerabilities = [
    {"id": "V-101", "name": "Exposed SSH Port", "severity": "CRITICAL", "layer": "L1", "status": "OPEN", "desc": "Port 22 detected as open to 0.0.0.0/0. Potential brute-force vector."},
    {"id": "V-102", "name": "Insecure Protocol", "severity": "HIGH", "layer": "L1", "status": "ACTIVE", "desc": "Telnet (Port 23) traffic detected in Layer 1 ingestion."},
    {"id": "V-103", "name": "Unsigned Binary Execution", "severity": "MEDIUM", "layer": "L3", "status": "DETECTED", "desc": "L3 Correlation identified unauthorized process startup."},
]
...
@app.post("/api/vulnerabilities/fix")
async def fix_vulnerabilities(req: dict):
    global vulnerabilities
    fix_id = req.get("id")
    if fix_id == "all":
        vulnerabilities = []
        msg = "All security vulnerabilities patched and ports closed."
    else:
        vulnerabilities = [v for v in vulnerabilities if v["id"] != fix_id]
        msg = f"Vulnerability {fix_id} remediated."
